...
time_schedule_file = "data-points-old/mumps_Nit_time_step0.05.txt"
time_schedule_custom = [12.3] # list(np.linspace(0,10,100))
...
initialization_file = "data-points-old/u2.h5"
initial_time = 2550.7 # rough
initial_time_step = 0.5 # replace dt if load_initialization is true
...
u_n_add = Function(V)  # The previous solution but in the same function space as u
...
eta_add, mu_add, gamma_e_add, gamma_h_add, phi_add, T_add, integral_phi_add = split(u)
...
fa.assign(u_n_add, [phi_0, T_0, eta_0, mu_0, gamma_e0, gamma_h0, integral_phi_0])
...
Fe_pc = (NC*dFermi_b(gamma_e)*(gamma_e - gamma_en)/delt \
        - K0*mu**2*(nd_eeq_pc*nd_heq_pc - nd_e*nd_h))*v_3*dx - dot(j_e_pcpnp, grad(v_3))*dx \
        -1.0/epsilon*(gamma_e*KB*T_n +CHI*mu**2/2 - CHP_IN )*v_3*ds(0) \
        -1.0/epsilon*(gamma_e*KB*T_n +CHI*mu**2/2 - CHP_IN )*v_3*ds(1)
...
#gamma_e last variable, nothig to change
Fh_pc = (NV*dFermi_b(gamma_h)*(gamma_h - gamma_hn)/delt \
      - K0*mu**2*(nd_eeq_pc*nd_heq_pc - nd_e*nd_h))*v_4*dx - dot(j_h_pcpnp, grad(v_4))*dx \
      -1.0/epsilon*(gamma_h*KB*T_n +CHI*mu**2/2 - CHP_IN )*v_4*ds(0) \
      -1.0/epsilon*(gamma_h*KB*T_n +CHI*mu**2/2 - CHP_IN )*v_4*ds(1)

Jy_pcpnp =ECHARGE*(j_hy_pcpnp - j_ey_pcpnp) 
Fbc_phi_Nitsche_pc = -1/epsilon*(phi + Rresistor*thickness*integral_phi- delV)*v_5*ds(0) \
                  + (integral_phi - Lx * Jy_pcpnp)*v_10*ds(0)
Fphi_pc = EPSILON*dot(grad(phi), grad(v_5))*dx - ECHARGE*(nd_h - nd_e)*v_5*dx + Fbc_phi_Nitsche_pc


FT_pc = FT

F_pc = Fphi_pc + FT_pc + Feta_pc + Fmu_pc + Fe_pc + Fh_pc 
...
if adaptive_time: 
    while t < tf:
        ...
        while rerror > tol_tstep:
         ...
         #       t = t_n
            ...
            #Define the more accurate approximation here.
            if n_step > 0: 
                print(n_step)
                u2acc = project(u_n_add + (dudt_n + (u - u_n_add)/dt)*dt/2.0, V)
            else: # only first step 
                n_step += 1
                t += dt
                dudt_n = project((u - u_n_add)/dt, V)
                update_u_n(u)
                u_n_add.assign(u)
                save_sol(u)
                break

            error1, error2,error3, error4, error5, error6 = rel_err_L2(u_coarse_t, u2acc)
    ...
    for dt in time_schedule: 
        myfile1 = open(time_steps_file1,'a')
        solve4dt(dt) #u_n updated
        update_u_n(u)
        n_step += 1
        t += dt 
        if rank == 0: 
            print("User message ===============> current time: ",t," time step size: ",dt)
            myfile1.write("User message ===============> current time: "+str(t)+" time step size: "+str(dt))
        myfile1.close()
